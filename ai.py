...
def query_ollama(prompt, data_context):
    """
    Query Ollama API with a prompt that includes data context.
    Adjust the URL, model, and payload as needed for your Ollama configuration.
    """
    url = "http://localhost:11434/api/generate"  # Change if necessary
    full_prompt = f"CSV File Context (first 3 rows):\n{data_context}\n\nUser Query:\n{prompt}"
    payload = {
        "prompt": full_prompt,
...
# Optionally include data context: we provide the first 3 rows as context.
